<x-app-layout>
    ...
    <x-modal name="project-store-modal" id="project_store_modal">
        <form action="{{ route('projects.store') }}" method="post">
            <h2 class="text-center pt-5 text-lg">Create User</h2>
            @csrf
            <div class="bg-white px-4  pb-4 sm:p-6 sm:pb-4">
                <label class="font-medium text-gray-800">Title</label>
                <x-text-input type="text" class="w-full outline-none rounded bg-gray-100 p-2 mt-2 mb-3 shadow-lg" id="title"  name="title" placeholder="Enter user Project title" value="{{ old('title') }}"></x-text-input>
                <label class="font-medium text-gray-800">Description</label>
                <textarea class="w-full outline-none rounded bg-gray-100 p-2 mt-2 mb-3 shadow" rows="5" id="description" name="description" placeholder="Enter user Description">{{ old('description') }}</textarea>
            </div>
            <div class="bg-gray-200 px-4 py-3 text-right">
                <x-danger-button x-on:click="$dispatch('close')" id="project_create_modal_cancel_button">
                    {{ __('Cancel') }}
                </x-danger-button>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded",function() {
            ...
            ProjectCreateModalCancelButton.addEventListener('click',function () {
                ProjectTitle.value = '';
